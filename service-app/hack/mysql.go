...
func main() {
	...
	// %d -> int
	var err error
	psqlInfo := "root:root@tcp(127.0.0.1:3306)/users" // username:pass@tcp(127.0.0.1:3306)/dbname
...
func queryMultipleRecords() {
	...
	for rows.Next() { // return one row if present otherwise stop the loop
		...
		err = rows.Scan(&id, &firstName) // scanning  data from each row
...
//create table users (id int AUTO_INCREMENT primary key, age int, email varchar(100), first_name varchar(100), last_name varchar(100));
